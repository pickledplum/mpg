...
createCountryCompanyTables <- function(conn, config) {
    ...
    company.column.type <- c("VARCHAR(20) PRIMARY KEY NOT NULL UNIQUE", 
                             ...
                             "VARCHAR(10)",
                             "CHAR(3)")
    company.column.constraint <- c("FOREIGN KEY(exchange_code) REFERENCES exchange (exchange_code) ON DELETE NO ACTION ON UPDATE CASCADE",
    ...
    company.meta.fql <- c("FG_COMPANY_NAME",
                          ...
                          "FG_FACTSET_SECTOR",
                          "FF_MAJOR_IND_NAME",
                          "FF_MANOR_SUBIND_NAME",
                          "FE_COMPANY_INFO(ISIN)",
                          "FE_COMPANY_INFO(SEDOL)")
    
    company.meta.column.name <- c(
        "id", 
        "name", 
        "domicile_country",
    ...
    company.meta.column.type <- c("VARCHAR(20) PRIMARY KEY NOT NULL UNIQUE", 
                             "TEXT(100)", 
                             "TEXT(40)",
                             "CHAR(3)",
                             "TEXT(40)",
    ...
    company.meta.column.constraint <- c()
    company.meta.specs <- c(paste(ccompany.meta.column.name, company.meta.column.type), company.meta.column.constraint)
    tryCreateTableIfNotExists(conn, "temp", company.meta.specs)
    logger.info(paste("Created TEMP table:", paste(company.meta.specs, collapse=",")))
    ...
    for( begin in seq(1, length(universe), BINSIZE )){
        ...
        if( ncol(all_company) != length(company.meta.column.name) ) {
            stop(paste("Collapted company meta data within:", mini_universe))
        }
        colnames(all_company) <- company.meta.column.name 
