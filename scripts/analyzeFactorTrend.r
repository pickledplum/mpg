...
oldest_timestamp <- min(valid_months)
tokens <- as.integer(strsplit(oldest_timestamp, "-")[[1]])
oldest_yy <- tokens[1]
oldest_mm <- tokens[2]
latest_timestamp <- max(valid_months)
tokens <- as.integer(strsplit(latest_timestamp, "-")[[1]])
latest_yy <- tokens[1]
latest_mm <- tokens[2]

nmons <- length(months)
nbins <- 5
logger.debug(paste("Valid months: min,max,# - ", paste(oldest_timestamp, latest_timestamp, nmons),sep=","))
# iterate from the most recent to the oldest
for( timestamp in tail(valid_months,13) ){
    yy <- rep(0,nmons)
    mm <- rep(0,nmons)
    
    # Test if there is 12 months into the future
    twelve_month_from_now <- incrementByMonth(yy,mm,12)
    next_yy <- tokens[1]
    next_mm <- tokens[2]
    if( !is.olderThanOrEqualTo( twelve_month_from_now, latest_timestamp ) ){
        logger.info(paste("No more 12 months in the future.  Done..."))
        break
    }
    ...
    r0 <- trc[ paste(yy0,mm0,sep="-")][1,]
    
    # Get the price for each company for this month
    p0 <- price[ paste(yy0,mm0,sep="-")][1,]
