...
get_fs_tseries <- function(stmt, ids, t0, t1, freq, curr, binsize) {
    ...
    for( begin in seq(1,length(ids), binsize) ) {
        ...
        a_company_data <- xts()
        tryCatch( {
            a_company_data <- get_a_series(stmt, ids[begin:end], t0, t1, freq, curr)
        }, error = function(msg) { 
        ...
        if( !wrote.header ){
            ...
            num.entries <- length(a_company_data)
            # Initialize the final data structure with the first entry
            tseries <- a_company_data
            # Write to the file, header first
            #write.table(t(c("ID",colnames(t(a_company_data)))), 
            #            file=temp_filepath, 
            #            sep=",", 
            #            row.names=FALSE, 
            #            col.names=FALSE)
            wrote.header <- TRUE
            #write.table(t(a_company_data), 
            #            file=temp_filepath, 
            #            append=TRUE, 
            #            sep=",", 
            #            row.names=TRUE, 
            ...
            tseries <- merge(tseries, a_company_data)
